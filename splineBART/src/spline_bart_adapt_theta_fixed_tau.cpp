...
// [[Rcpp::export(name = ".adapt_theta_fixed_tau")]]
Rcpp::List adapt_theta_fixed_tau(Rcpp::List Y_train,
                                 Rcpp::List Phi_train,
                                 arma::mat Z_train,
                                 arma::mat Z_test,
                                 Rcpp::List cutpoints,
                                 ...
                                 double a, double b, size_t N_u, double rho_eta, // hyperparameters for split probabilities
                                 ...
                                 bool verbose, size_t print_every, bool debug = false)
{
  ...
  double u_init = gen.beta(a,b);
  double eta = rho_eta * u_init/(1.0 - u_init); // initialize eta
  ...
  arma::mat theta_samples(p, nd+burn);
  ...
  for(size_t iter = 0; iter < nd + burn; iter++){
    ...
    // update split probabilities theta
    update_theta(theta, var_counts, eta, p, gen);
    
    // update Dirichlet concentration parameter eta
    update_eta(eta, rho_eta, theta, p, N_u, a, b, gen);
    
    
    sigma_samples(iter) = sigma; // save the value of sigma
    for(size_t j = 0; j < p; j++) theta_samples(j,iter) = theta[j]; // save values of theta
